...
let mainInput = `febca cfagb ecbafd efdcbg cbegdfa fg bgafec gfae acgdb gfc | cgf facdeb ecgfdb afcbge
...
mainInput = mainInput.replaceAll("| ", "|\n");

function countUniques(input) {
  let inputArr = input.split("\n");
  let uniques = 0;

  for (let i = 1; i < inputArr.length; i += 2) {
    let outputVals = inputArr[i].split(" ");
    outputVals.forEach(val => {
      let l = val.length;
      if (l == 2 || l == 4 || l == 3 || l == 7) uniques++;
    })
  }
  return uniques;
...
console.log(countUniques(testInput));
console.log(countUniques(mainInput));
...
function sumOutputVals(input) {
  let inputArr = input.split("\n");
  let sum = 0;
  for (let i = 0; i < inputArr.length; i += 2) {
    sum += outputVal(inputArr[i].slice(0, inputArr[i].length - 2), inputArr[i + 1]);
  }
  return sum;
}

// returns 4-digit decimal output from an entry line of the input.
// clues = string.  part of entry before |
// outputDigits = string.  part of entry after |
function outputVal(clues, outputDigits) {
  clues = clues.split(" ");
  clues.sort((x, y) => {
  ...
  // 1, 7, 4, and 8 can be identified because they light a unique number of 
  // segments.
  let set1 = new Set(clues[0].split(""));
  let set7 = new Set(clues[1].split(""));
  let set4 = new Set(clues[2].split(""));
  let set8 = new Set(clues[9].split(""));

  // remaining sets can be found with set operations.
  let five1 = new Set(clues[3].split(""));
  let five2 = new Set(clues[4].split(""));
  let five3 = new Set(clues[5].split(""));
  let six1 = new Set(clues[6].split(""));
  let six2 = new Set(clues[7].split(""));
  let six3 = new Set(clues[8].split(""));
  
  let int235 = new Set(intersection(five1, intersection(five2, five3)));
  let int069 = new Set(intersection(six1, intersection(six2, six3)));
  
  let set0 = union(compliment(int235), compliment(set4));
  let set2 = union(int235, compliment(int069));
  let set3 = union(set1, int235);
  let set5 = union(int235, int069);
  let set6 = union(compliment(set1), compliment(set2));
  let set9 = union(set4, set3);

  // now parse output digits by matching to correct sets.
  // the answer key is a map.  keys of the map are strings.
  // values are string representations of numbers 0 through 9.
  let sets = [
    Array.from(set0.values()),
    Array.from(set1.values()),
    Array.from(set2.values()),
    Array.from(set3.values()),
    Array.from(set4.values()),
    Array.from(set5.values()),
    Array.from(set6.values()),
    Array.from(set7.values()),
    Array.from(set8.values()),
    Array.from(set9.values())
  ];
  sets.forEach(set => set.sort());
  sets = sets.map(set => set.join(""));
  
  const key = new Map();
  sets.forEach((elt, index) => key.set(elt, index.toString()));

  outputDigits = outputDigits.split(" ").map(e => e.split("").sort().join(""));
  let _outputVal = "";
  outputDigits.forEach(digit => _outputVal += key.get(digit));
  return Number(_outputVal);
}
...
function compliment(setA) {
  return difference(SEGMENTS, setA);
}
...
console.log(sumOutputVals(testInput));
console.log(sumOutputVals(mainInput));